...
public final class IpTable {
    ...
    public static boolean assertBinaries(Context ctx, boolean showErrors) {
        ...
        try {
            ...
            if (changed) {
                Toast.makeText(ctx, "未知toast", Toast.LENGTH_LONG).show();
            }
        } catch (Exception e) {
            if (showErrors)
    ...
    public static void setEnabled(Context ctx, boolean enabled) {
		...
		/* notify */
        final Intent message = new Intent(IpTable.STATUS_CHANGED_MSG);
        message.putExtra(IpTable.STATUS_EXTRA, enabled);
